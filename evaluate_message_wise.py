...
class Evaluator:
    ...
    def init_from_dir(self, dir):
        ...
        num_asser, num_user = len(self.asserlist), len(self.namelist)
        print("Num assertion: {}, Num user: {}".format(num_asser, num_user))
        # Recover tweet
        with open(dir + "/" + "tweet_to_assertion_id_map.json", 'r') as fin:
            tweet_to_asser = json.load(fin)
            assert_to_tweet = {}
            for k, v in tweet_to_asser.items():
                if v not in assert_to_tweet:
                    assert_to_tweet[v] = []
                else:
                    assert_to_tweet[v].append(k)
        attach_tweet_embeddings = []
        attach_tweet_label = []
        attach_asserlist = []
        for i in range(num_asser):
            if len(assert_to_tweet[i]) > 1:
                for k in range(len(assert_to_tweet[i]) - 1):
                    attach_tweet_embeddings.append(self.embedding[num_user + i].reshape(1, -1))
                    attach_tweet_label.append(self.labels["assertion_label"][i])
                    attach_asserlist.append(assert_to_tweet[i][1 + k])
        self.embedding = np.concatenate([self.embedding, np.concatenate(attach_tweet_embeddings, axis=0)], axis=0)
        self.labels["assertion_label"] = np.concatenate([self.labels["assertion_label"], np.array(attach_tweet_label).astype("int32")], axis=0)
        self.asserlist.extend(attach_asserlist)
        print("Num Tweet: {}".format(len(self.asserlist)))
    ...
    def numerical_evaluate(self):
        ...
        print("evaluated size: {}".format(asser_label.shape[0]))
...
if __name__ == "__main__":
    for d in [
        "/home/jinning4/work/incas/SGVGAE_2b/2B/belief_embedding_output_defense_2_0.50_0.50",
        "/home/jinning4/work/incas/SGVGAE_2b/2B/belief_embedding_output_domestic-economy_2_0.50_0.50",
        "/home/jinning4/work/incas/SGVGAE_2b/2B/belief_embedding_output_insurgency-separatism_2_0.50_0.50",
        "/home/jinning4/work/incas/SGVGAE_2b/2B/belief_embedding_output_territorial-sovereignty_2_0.50_0.50",
        "/home/jinning4/work/incas/SGVGAE_2b/2B/belief_embedding_output_trade_2_0.50_0.50",
        "/home/jinning4/work/incas/SGVGAE_2b/2B/belief_embedding_output_us_military_2_0.50_0.50"
    ]:
        print(d)
        ...
        with open("log.txt", "a") as fout:
            fout.write("{:.2f}%\t{:.2f}%\t{:.2f}%\t\t{}\t{}\n".format(
            100*acc, 100*macro_f1, 100*avg_purity,
            len(evaluator.asserlist), len(evaluator.asserlist)
        ))
