...
client = OpenAI(
    ...
    api_key="sk-3sPg07pC2qc1wtXo5VhgBmI-rgGn_-_ih_E4kfoF-IT3BlbkFJB94jL6ib2ImXolL5UWeivn9b7V3AtJKGpMTm7uAVkA",
)
...
def get_visualization_data(unique_id):
    ...
    for meaning_chunk in get_subsequences(label_data["belief_axis_meaning"]):
        belief_caption_data.append("[Issue]: " + ", ".join(meaning_chunk))
    ...
    for k in range(belief_dim):
        polar_caption_data.append(
            "{}".format("[Polarization on " + label_data["belief_axis_meaning"][k]) + "]: <br>" + ", ".join(label_data["polar_axis_meaning"][k]))
